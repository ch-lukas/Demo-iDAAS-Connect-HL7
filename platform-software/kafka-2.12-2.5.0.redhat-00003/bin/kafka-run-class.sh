...
if [ "x$LOG_DIR" = "x" ]; then
  ...
  LOG_DIR=$(echo $(cygpath -wp $base_dir/logs))
fi
...
# Log4j settings
  ...
  #LOG4J_DIR=$(echo $(cygpath -wp $base_dir/config/tools-log4j.properties))
...
KAFKA_LOG4J_OPTS="-Dlog4j.configuration=file:$(echo $(cygpath -wp $base_dir/config/tools-log4j.properties)) -Dkafka.logs.dir=$(echo $(cygpath -wp $base_dir/logs))"
...
#fi
JAVA="./platform-software/java-1.8.0/bin/java"
...
CLASSPATH=$(echo $(cygpath -wp $CLASSPATH))
